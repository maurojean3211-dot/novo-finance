-- PREFLIGHT ESTRITAMENTE SOMENTE LEITURA. Todos os campos *_ok devem retornar true antes do hardening.

select count(*) total,count(*) filter(where status='Pago') pagos,
 count(*) filter(where status='Pendente') pendentes,coalesce(sum(valor),0) total_nominal,
 md5(string_agg(to_jsonb(p)::text,'|' order by id)) fingerprint_dados
from public.contas_pagar_pessoais p;

select c.relrowsecurity,c.relforcerowsecurity,r.rolname owner_role,r.rolbypassrls owner_bypassrls,c.relacl
...
select
 ...
 (select coalesce(sum(valor),0) from public.contas_pagar_pessoais)=41574.73 total_41574_73_ok,
 ...
 (select md5(coalesce(string_agg(to_jsonb(x)::text,'|' order by policyname),'')) from (
  ...
  ='1a4a637e8a62c5d9c6de391deffdb43d' policies_fingerprint_ok,
 ...
 (select bool_and(has_table_privilege('authenticated','public.contas_pagar_pessoais',p))
  from unnest(array['TRUNCATE','REFERENCES','TRIGGER','MAINTAIN']) p) authenticated_quatro_excessos_presentes_ok,
 ...
 (select bool_and(has_table_privilege('service_role','public.contas_pagar_pessoais',p))
  from unnest(array['SELECT','INSERT','UPDATE','DELETE','TRUNCATE','REFERENCES','TRIGGER','MAINTAIN']) p) service_role_preservado_ok,
 (select bool_and(has_table_privilege('postgres','public.contas_pagar_pessoais',p))
  from unnest(array['SELECT','INSERT','UPDATE','DELETE','TRUNCATE','REFERENCES','TRIGGER','MAINTAIN']) p) postgres_preservado_ok,
 (select md5(coalesce(string_agg(concat(conname,':',contype,':',pg_get_constraintdef(oid)),'|' order by conname),''))
  ...
  ='feb963cf66a3fdf8c703cf0b3c188679' constraints_ok,
 (select md5(coalesce(string_agg(concat(indexname,':',indexdef),'|' order by indexname),''))
  ...
  ='4849d3d3ae3570eb137eb85dc1d5fe58' indices_ok,
