...
begin
  ...
  if exists (
    ...
    where table_schema = 'public' and table_name = 'contas_fixas' and column_name = 'proprietario_id'
  ) then
    if exists (
      select 1
      from public.contas_fixas c
      where (
        c.proprietario_id is null
        and (select count(*) from public.usuarios u where u.empresa_id = c.empresa_id) <> 1
      ) or (
        c.proprietario_id is not null
        and not exists (
          select 1 from public.usuarios u
          where u.id = c.proprietario_id and u.empresa_id = c.empresa_id
        )
      )
    ) then
      raise exception 'ABORTADO: ownership existente/projetado de conta fixa é inválido';
    end if;
  elsif exists (
    select 1 from public.contas_fixas c
...
-- Aceita exclusivamente o baseline real de produção ou o estado já homologado.
-- Qualquer terceiro formato aborta antes do backfill financeiro.
do $trigger_preflight$
declare
  v_trigger record;
  v_is_production_baseline boolean;
  v_is_homologated_constraint boolean;
begin
  if to_regprocedure('public.validar_despesa_evento_pessoal()') is null
     or md5(pg_get_functiondef(to_regprocedure('public.validar_despesa_evento_pessoal()')))
        <> 'ab8cf89ec4cdd9e17fc0f2b13f3172f4' then
    raise exception 'ABORTADO: função de integridade de despesas ausente ou divergente';
  end if;
  ...
  where n.nspname = 'public'
    and c.relname = 'despesas'
    and t.tgname = 'despesas_evento_pessoal_integridade'
    and not t.tgisinternal;

  if not found then
    raise exception 'ABORTADO: trigger despesas_evento_pessoal_integridade ausente';
  end if;

  v_is_production_baseline :=
    not v_trigger.tgdeferrable
    and not v_trigger.tginitdeferred
    and v_trigger.tgconstraint = 0
    and v_trigger.definition =
      'CREATE TRIGGER despesas_evento_pessoal_integridade BEFORE INSERT OR DELETE OR UPDATE ON public.despesas FOR EACH ROW EXECUTE FUNCTION validar_despesa_evento_pessoal()';

  v_is_homologated_constraint :=
    v_trigger.tgdeferrable
    and v_trigger.tginitdeferred
    and v_trigger.tgconstraint <> 0
    and v_trigger.definition =
      'CREATE CONSTRAINT TRIGGER despesas_evento_pessoal_integridade AFTER INSERT OR UPDATE ON public.despesas DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION validar_despesa_evento_pessoal()';

  if not v_is_production_baseline and not v_is_homologated_constraint then
    raise exception 'ABORTADO: configuração inesperada de despesas_evento_pessoal_integridade: %',
      v_trigger.definition;
  end if;

  if v_is_production_baseline then
    drop trigger despesas_evento_pessoal_integridade on public.despesas;
    create constraint trigger despesas_evento_pessoal_integridade
      after insert or update on public.despesas
      deferrable initially deferred
      for each row execute function public.validar_despesa_evento_pessoal();
  end if;
end $trigger_preflight$;

do $trigger_post_conversion$
begin
  if not exists (
    select 1
    from pg_trigger t
    ...
    raise exception 'ABORTADO: conversão do trigger de integridade não atingiu o estado homologado';
  end if;
end $trigger_post_conversion$;
...
set constraints public.despesas_evento_pessoal_integridade immediate;
