...
create table public.financeiro_baixas (
  ...
  created_at timestamptz not null default now(),
  unique (id, empresa_id),
  foreign key (titulo_id, empresa_id) references public.financeiro_titulos(id, empresa_id) on update cascade on delete restrict,
  foreign key (estorno_de, empresa_id) references public.financeiro_baixas(id, empresa_id) on update cascade on delete restrict
);
...
create table public.financeiro_conciliacoes (
  ...
  foreign key (titulo_id, empresa_id) references public.financeiro_titulos(id, empresa_id) on update cascade on delete restrict,
  foreign key (baixa_id, empresa_id) references public.financeiro_baixas(id, empresa_id) on update cascade on delete restrict
);
...
create policy "financeiro_titulos_select_empresa" on public.financeiro_titulos for select to authenticated
  using (exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_titulos.empresa_id));
create policy "financeiro_titulos_insert_empresa" on public.financeiro_titulos for insert to authenticated
  with check (user_id=auth.uid() and exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_titulos.empresa_id));
create policy "financeiro_titulos_update_empresa" on public.financeiro_titulos for update to authenticated
  using (exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_titulos.empresa_id))
  with check (exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_titulos.empresa_id));

create policy "financeiro_baixas_select_empresa" on public.financeiro_baixas for select to authenticated
  using (exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_baixas.empresa_id));
create policy "financeiro_conciliacoes_empresa" on public.financeiro_conciliacoes for select to authenticated
  using (exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_conciliacoes.empresa_id));
create policy "financeiro_conciliacoes_insert_empresa" on public.financeiro_conciliacoes for insert to authenticated
  with check (user_id=auth.uid() and exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_conciliacoes.empresa_id));
create policy "financeiro_conciliacoes_update_empresa" on public.financeiro_conciliacoes for update to authenticated
  using (exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_conciliacoes.empresa_id))
  with check (exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_conciliacoes.empresa_id));
create policy "financeiro_historico_select_empresa" on public.financeiro_historico for select to authenticated
  using (exists (select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id=financeiro_historico.empresa_id));

create or replace function public.registrar_titulo_financeiro(
  p_empresa_id uuid,p_tipo text,p_contraparte_nome text,p_origem text,p_origem_id text,p_referencia text,
...
create or replace function public.baixar_titulo_financeiro(
  p_titulo_id uuid,p_empresa_id uuid,p_valor numeric,p_data date,p_forma text,p_conta text,p_observacoes text default null
) returns uuid language plpgsql security definer set search_path='' as $$
...
create or replace function public.editar_titulo_financeiro(
  p_titulo_id uuid,p_empresa_id uuid,p_contraparte_nome text,p_referencia text,p_descricao text,
...
create or replace function public.conciliar_titulo_financeiro(
  p_titulo_id uuid,p_empresa_id uuid,p_conta text,p_data date,p_valor numeric,p_status text,p_observacoes text default null
) returns uuid language plpgsql security definer set search_path='' as $$
...
create or replace function public.estornar_baixa_financeira(
  p_baixa_id uuid,p_empresa_id uuid,p_data date,p_observacoes text default null
) returns uuid language plpgsql security definer set search_path='' as $$
...
revoke execute on function public.registrar_titulo_financeiro(uuid,text,text,text,text,text,text,date,numeric,text,text,text,text) from public;
revoke execute on function public.baixar_titulo_financeiro(uuid,uuid,numeric,date,text,text,text) from public;
revoke execute on function public.editar_titulo_financeiro(uuid,uuid,text,text,text,text,text,date,text) from public;
revoke execute on function public.conciliar_titulo_financeiro(uuid,uuid,text,date,numeric,text,text) from public;
revoke execute on function public.estornar_baixa_financeira(uuid,uuid,date,text) from public;
...
grant execute on function public.baixar_titulo_financeiro(uuid,uuid,numeric,date,text,text,text) to authenticated;
grant execute on function public.editar_titulo_financeiro(uuid,uuid,text,text,text,text,text,date,text) to authenticated;
grant execute on function public.conciliar_titulo_financeiro(uuid,uuid,text,date,numeric,text,text) to authenticated;
...
comment on function public.registrar_titulo_financeiro(uuid,text,text,text,text,text,text,date,numeric,text,text,text,text) is 'Registro confirmado pelo usuário, com isolamento por empresa e deduplicação por origem.';
