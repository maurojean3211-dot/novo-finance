create table if not exists public.pedidos_compra (
 ...
 aprovado_por uuid references auth.users(id) on update cascade on delete restrict, aprovado_em timestamptz, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique(empresa_id,numero)
);
create table if not exists public.pedido_compra_itens (
 id uuid primary key default gen_random_uuid(), pedido_id uuid not null references public.pedidos_compra(id) on update cascade on delete restrict, empresa_id text not null,
 produto_id text not null, estoque_id uuid references public.estoque(id) on update cascade on delete restrict, produto text not null, descricao text, liga text, tempera text, dimensao text,
...
create table if not exists public.pedido_compra_cotacoes (
 id uuid primary key default gen_random_uuid(), pedido_id uuid not null references public.pedidos_compra(id) on update cascade on delete restrict, empresa_id text not null, fornecedor_id text not null,
...
create table if not exists public.pedido_compra_historico (
 id uuid primary key default gen_random_uuid(), pedido_id uuid not null references public.pedidos_compra(id) on update cascade on delete restrict, empresa_id text not null, user_id uuid not null references auth.users(id) on update cascade on delete restrict,
...
create table if not exists public.pedido_compra_parcelas (
 id uuid primary key default gen_random_uuid(), pedido_id uuid not null references public.pedidos_compra(id) on update cascade on delete restrict, empresa_id text not null,
...
create policy "pedidos_compra_empresa" on public.pedidos_compra for all to authenticated using(exists(select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id::text=pedidos_compra.empresa_id)) with check(exists(select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id::text=pedidos_compra.empresa_id));
...
create policy "pedido_historico_select" on public.pedido_compra_historico for select to authenticated using(exists(select 1 from public.pedidos_compra p where p.id=pedido_compra_historico.pedido_id and p.empresa_id=pedido_compra_historico.empresa_id));
create policy "pedido_historico_insert" on public.pedido_compra_historico for insert to authenticated with check(user_id=auth.uid() and exists(select 1 from public.pedidos_compra p where p.id=pedido_compra_historico.pedido_id and p.empresa_id=pedido_compra_historico.empresa_id));
...
create or replace function public.receber_item_pedido(p_item_id uuid,p_empresa_id text,p_quantidade numeric)
...
begin
 ...
 update public.pedido_compra_itens set quantidade_recebida=v_novo where id=p_item_id;
 select count(*) into v_pendentes from public.pedido_compra_itens where pedido_id=v_pedido.id and quantidade_recebida<quantidade;
 update public.pedidos_compra set status=case when v_pendentes=0 then 'Recebido' else 'Recebido parcialmente' end,updated_at=now() where id=v_pedido.id;
...
grant execute on function public.receber_item_pedido(uuid,text,numeric) to authenticated;
