create table if not exists public.ordens_producao (
  id uuid primary key default gen_random_uuid(), empresa_id text not null,
  ...
  orcamento_id uuid references public.orcamentos(id) on update cascade on delete restrict,
  ...
  produto_acabado_estoque_id uuid references public.estoque(id) on update cascade on delete restrict,
  entrada_produto_acabado_em timestamptz,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
  unique(empresa_id,numero_op), check(quantidade_produzida+quantidade_perdida<=quantidade_planejada)
);

create table if not exists public.ordem_producao_materiais (
...
create table if not exists public.ordem_producao_apontamentos (
  id uuid primary key default gen_random_uuid(), ordem_id uuid not null references public.ordens_producao(id) on update cascade on delete restrict,
...
create table if not exists public.ordem_producao_historico (
  id uuid primary key default gen_random_uuid(), ordem_id uuid not null references public.ordens_producao(id) on update cascade on delete restrict,
  empresa_id text not null, user_id uuid not null references auth.users(id) on update cascade on delete restrict,
...
create policy "ordens_producao_select_empresa" on public.ordens_producao for select to authenticated
 using(exists(select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id::text=ordens_producao.empresa_id));
create policy "ordens_producao_insert_empresa" on public.ordens_producao for insert to authenticated
 with check(user_id=auth.uid() and exists(select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id::text=ordens_producao.empresa_id));
create policy "ordens_producao_update_empresa" on public.ordens_producao for update to authenticated
 using(exists(select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id::text=ordens_producao.empresa_id))
 with check(exists(select 1 from public.usuarios u where u.id=auth.uid() and u.empresa_id::text=ordens_producao.empresa_id));
create policy "ordem_materiais_empresa" on public.ordem_producao_materiais for all to authenticated
 using(exists(select 1 from public.ordens_producao o where o.id=ordem_producao_materiais.ordem_id and o.empresa_id=ordem_producao_materiais.empresa_id))
 with check(exists(select 1 from public.ordens_producao o where o.id=ordem_producao_materiais.ordem_id and o.empresa_id=ordem_producao_materiais.empresa_id));
create policy "ordem_apontamentos_select" on public.ordem_producao_apontamentos for select to authenticated
 using(exists(select 1 from public.ordens_producao o where o.id=ordem_producao_apontamentos.ordem_id and o.empresa_id=ordem_producao_apontamentos.empresa_id));
create policy "ordem_apontamentos_insert" on public.ordem_producao_apontamentos for insert to authenticated
 with check(user_id=auth.uid() and exists(select 1 from public.ordens_producao o where o.id=ordem_producao_apontamentos.ordem_id and o.empresa_id=ordem_producao_apontamentos.empresa_id));
create policy "ordem_historico_select" on public.ordem_producao_historico for select to authenticated
 using(exists(select 1 from public.ordens_producao o where o.id=ordem_producao_historico.ordem_id and o.empresa_id=ordem_producao_historico.empresa_id));
create policy "ordem_historico_insert" on public.ordem_producao_historico for insert to authenticated
 with check(user_id=auth.uid() and exists(select 1 from public.ordens_producao o where o.id=ordem_producao_historico.ordem_id and o.empresa_id=ordem_producao_historico.empresa_id));

create or replace function public.reservar_material_producao(p_material_id uuid,p_empresa_id text,p_quantidade numeric)
...
begin
 ...
 update public.ordem_producao_materiais set quantidade_reservada=v_novo,necessidade_compra=false,updated_at=now() where id=v_material.id;
...
create or replace function public.liberar_material_producao(p_material_id uuid,p_empresa_id text,p_quantidade numeric)
returns void language plpgsql security invoker set search_path=public as $$
declare v_material public.ordem_producao_materiais;v_ordem public.ordens_producao;v_novo numeric;
begin
 if p_quantidade<=0 then raise exception 'Quantidade inválida.';end if;
 ...
 update public.ordem_producao_materiais set quantidade_reservada=v_novo,updated_at=now() where id=v_material.id;
...
create or replace function public.reverter_consumo_producao(p_material_id uuid,p_empresa_id text,p_movimentacao_id uuid)
returns void language plpgsql security invoker set search_path=public as $$
...
begin
 ...
 update public.ordem_producao_materiais set quantidade_consumida=quantidade_consumida-v_mov.quantidade,updated_at=now() where id=v_material.id;
...
create or replace function public.entrar_produto_acabado(p_ordem_id uuid,p_empresa_id text,p_estoque_id uuid,p_quantidade numeric)
returns void language plpgsql security invoker set search_path=public as $$
...
begin
 ...
 update public.ordens_producao set produto_acabado_estoque_id=p_estoque_id,entrada_produto_acabado_em=now(),updated_at=now() where id=p_ordem_id;
...
grant execute on function public.reservar_material_producao(uuid,text,numeric) to authenticated;
grant execute on function public.consumir_material_producao(uuid,text,numeric) to authenticated;
grant execute on function public.liberar_material_producao(uuid,text,numeric) to authenticated;
grant execute on function public.reverter_consumo_producao(uuid,text,uuid) to authenticated;
grant execute on function public.entrar_produto_acabado(uuid,text,uuid,numeric) to authenticated;
