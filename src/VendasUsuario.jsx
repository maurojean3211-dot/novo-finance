...
export default function VendasUsuario({ empresaId, userId, companyName = "Cunha Finance", issuedBy = "Não informado" }) {
  ...
  const totalValor = vendasFiltradas.reduce((total, v) => total + Number(v.valor || 0), 0);
  const totalQuantidade = vendasFiltradas.reduce((total, v) => total + Number(v.kilos || 0), 0);
  const totalComissao = vendasFiltradas.reduce((total, v) => total + getStoredOrCalculatedCommission(v), 0);
  ...
  return <div className="ops-page">
    ...
    <MetricGrid items={[{ label: "Vendas do período", value: vendasFiltradas.length, detail: "registros filtrados", icon: "▥" }, { label: "Peso vendido", value: `${totalQuantidade.toLocaleString("pt-BR")} kg`, detail: "quantidade total", icon: "⚖", tone: "green" }, { label: "Valor total", value: `R$ ${totalValor.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, detail: "valor acumulado", icon: "R$", tone: "amber" }, { label: "Comissão", value: `R$ ${totalComissao.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, detail: "comissão acumulada", icon: "%" }, { label: "Quantidade", value: vendasFiltradas.length, detail: "vendas exibidas", icon: "#" }]} />
    ...
    <section className="ops-panel"><div className="ops-panel__header"><h2>Registro de vendas</h2><span>{vendasFiltradas.length} resultado(s)</span></div>{vendasFiltradas.length === 0 ? <EmptyState /> : <div className="ops-table-wrap"><table className="ops-table"><thead><tr><th>Data</th><th>Cliente</th><th>Produto</th><th>Peso</th><th>Valor</th><th>Comissão</th><th>Vendedor</th><th>Ações</th></tr></thead><tbody>{vendasFiltradas.map((v) => <tr key={v.id}><td>{formatarData(v.data_venda)}</td><td><strong>{v.cliente_nome || "-"}</strong></td><td>{v.produto || "-"}</td><td>{Number(v.kilos || 0).toLocaleString("pt-BR")} kg</td><td>R$ {Number(v.valor || 0).toFixed(2)}</td><td>R$ {getStoredOrCalculatedCommission(v).toFixed(2)}</td><td>{v.vendedor_nome || v.vendedor || "—"}</td><td><ActionButtons onEdit={() => editarVenda(v)} onDelete={() => excluirVenda(v.id)} /></td></tr>)}</tbody></table></div>}</section>
