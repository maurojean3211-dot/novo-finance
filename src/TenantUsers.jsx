...
import { isCurrentTeamUser, tenantProfileLabel } from "./teamRoles";
...
export default function TenantUsers() {
  ...
  return <main className="tenant-users ops-page">
    ...
    <section className="ops-panel"><div className="ops-panel__header"><h2>Equipe</h2><span>Somente sua empresa</span></div><div className="ops-table-wrap"><table className="ops-table"><thead><tr><th>Usuário</th><th>Perfil</th><th>Permissões</th><th>Status</th><th>Ações</th></tr></thead><tbody>{users.map((user) => { const currentUser = isCurrentTeamUser(user, session?.user?.id); return <tr key={user.id}><td>{user.nome}<small>{user.email}</small></td><td>{tenantProfileLabel(user, profiles)}{currentUser && <small>Conta atual</small>}</td><td>{Object.entries(user.permissoes || {}).filter(([,v]) => v).map(([k]) => k).join(", ") || "Sem módulos"}</td><td>{user.status}</td><td><div className="tenant-users__actions">{currentUser ? <span className="tenant-users__self-protected">Conta protegida</span> : <><button onClick={() => setEditing({ ...user, perfil: user.nivel || "personalizado" })}>Editar</button>{user.status === "ATIVO" ? <button disabled={busy} onClick={() => run(() => blockTenantUser(user.id), "Usuário bloqueado.")}>Bloquear</button> : <button disabled={busy} onClick={() => run(() => unblockTenantUser(user.id), "Usuário ativado.")}>Ativar</button>}<button disabled={busy} onClick={() => confirm("Remover o vínculo deste usuário?") && run(() => removeTenantUser(user.id), "Vínculo removido.")}>Remover</button></>}</div></td></tr>; })}</tbody></table></div></section>
