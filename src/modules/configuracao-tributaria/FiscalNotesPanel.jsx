...
export default function FiscalNotesPanel({ empresaId, userId, configurations, rules }) {
  ...
  const inputRef = useRef(null); const version = useRef(0);
  const load = useCallback(async () => {
    const current = ++version.current;
    if (!empresaId) { setNotes([]); setSelected(null); return; }
    try { const data = await listFiscalNotes(empresaId); if (current === version.current) { setNotes(data); setSelected((previous) => data.find((item) => item.id === previous?.id) ?? null); } }
    catch (error) { if (current === version.current) setFeedback(error.message); }
  }, [empresaId]);
  useEffect(() => { setFeedback(""); load(); }, [load]);

  async function upload(event) {
    const file = event.target.files?.[0]; event.target.value = ""; if (!file) return;
    setBusy(true); setFeedback("Lendo e conferindo a nota fiscal…");
    try { await importFiscalNote({ file, empresaId, configurations, rules }); await load(); setFeedback("Nota fiscal importada e analisada."); }
    catch (error) { setFeedback(error.message); } finally { setBusy(false); }
  }
  async function review(note) { setBusy(true); try { await markFiscalNoteReviewed(empresaId, note.id, userId); await load(); setFeedback("Nota marcada como revisada."); } catch (error) { setFeedback(error.message); } finally { setBusy(false); } }
  async function remove(note) {
    if (!window.confirm(`Excluir somente a nota ${note.numero || "sem número"}?`)) return;
