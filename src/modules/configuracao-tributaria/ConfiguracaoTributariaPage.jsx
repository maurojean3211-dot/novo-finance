...
export default function ConfiguracaoTributariaPage({ empresaId, userId }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const nextSituation = await loadTaxSituation(empresaId, nextRecords);
  ...
  async function submit(event) {
    ...
    setSaving(true);
    try {
      await saveTaxConfiguration({ empresaId, userId, configuration: form });
      setForm(initialForm);
      await load();
  ...
  return (
    <main className="ops-page tax-config-page">
      ...
      <section className="ops-panel tax-situation" aria-labelledby="tax-situation-title">
        <div className="tax-situation__heading"><div><small>Situação tributária</small><h2 id="tax-situation-title">Visão atual da empresa</h2></div><span className={`tax-situation__status tax-situation__status--${status.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")}`}>{status}</span></div>
        <dl>
          ...
          <div><dt>Última verificação das regras</dt><dd>{situation.checkedAt ? new Date(situation.checkedAt).toLocaleString("pt-BR") : "Pendente"}</dd></div>
      ...
      <div className="tax-config-layout">
        <form className="ops-panel tax-config-form" onSubmit={submit}>
          ...
          <button type="submit" disabled={saving}>{saving ? "Salvando…" : "Registrar configuração"}</button>
      ...
      <FiscalNotesPanel empresaId={empresaId} userId={userId} configurations={records} rules={situation.rules ?? []} />
