...
function ComprasWorkspace({ empresaId, userId }) {
  ...
  return <main className="ops-page smart-purchases">
    ...
    <section className="ops-panel"><div className="ops-panel__header"><h2>Pedidos de compra</h2><span>{manager.orders.length} pedido(s)</span></div><div className="ops-table-wrap"><table className="ops-table"><thead><tr><th>Número</th><th>Fornecedor</th><th>Status</th><th>Data</th><th>Previsão</th><th>Itens</th><th>Total</th><th>Ações</th></tr></thead><tbody>{manager.orders.map((row) => <tr key={row.id}><td><strong>{row.numero}</strong></td><td>{row.fornecedor.nome}</td><td><select value={row.status} disabled={!allowedStatusTransitions(row.status).length} onChange={async (event) => { const status = event.target.value; if (!window.confirm(`Confirmar mudança transacional para ${status}?`)) return; try { await manager.status(row, status); setFeedback(`Status alterado para ${status} com histórico.`); } catch (error) { setFeedback(error.message); } }}><option value={row.status}>{row.status}</option>{allowedStatusTransitions(row.status).map((status) => <option key={status}>{status}</option>)}</select></td><td>{row.data}</td><td>{row.previsao || "—"}</td><td>{row.items.length}</td><td>{money(row.valorTotal)}</td><td><button onClick={() => setSelected(row)}>Detalhes</button></td></tr>)}</tbody></table></div></section>
...
function HistoricalPurchases({ rows, onSelect, onEdit, onDelete }) {
  return <section className="ops-panel"><div className="ops-panel__header"><div><h2>Compras históricas</h2><small>Fonte legada. Correções exigem confirmação e não executam ações operacionais.</small></div><span>{rows.length} registro(s)</span></div>{!rows.length ? <p>Nenhuma compra histórica visível para esta empresa.</p> : <div className="ops-table-wrap"><table className="ops-table"><thead><tr><th>Origem</th><th>Data</th><th>Fornecedor</th><th>Produto</th><th>Quantidade</th><th>Valor disponível</th><th>Ações</th></tr></thead><tbody>{rows.map((row) => <tr key={`legacy:${row.id}`}><td><strong>Compra histórica</strong></td><td>{String(row.data_compra || row.created_at || "—").slice(0, 10)}</td><td>{row.fornecedor || "—"}</td><td>{row.produto || row.material || "—"}</td><td>{Number(row.kilos || row.quantidade || 0).toLocaleString("pt-BR")}</td><td>{money(row.valor ?? row.valor_total ?? row.preco_compra ?? 0)}</td><td><ActionButtons onView={() => onSelect(row)} onEdit={() => onEdit(row)} onDelete={() => onDelete(row)} /></td></tr>)}</tbody></table></div>}</section>;
}
