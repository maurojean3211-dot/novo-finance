...
import { allowedStatusTransitions } from "../services/compras.service";
...
function ComprasWorkspace({ empresaId, userId }) {
  ...
  async function addProduct(product) {
    if (order.items.some((item) => String(item.produtoId) === String(product.id))) return setFeedback("Este produto já está no pedido. Ajuste sua quantidade no item existente.");
  ...
  async function save() {
    ...
    if (!window.confirm(order.id ? "Confirmar atualização transacional do pedido e seus itens?" : "Confirmar gravação transacional deste pedido de compra? Nenhum pedido é criado automaticamente pelo PCP.")) return;
    try {
      ...
      setModal(false); setOrder(empty()); setFeedback(order.id ? "Pedido e itens atualizados com histórico." : "Pedido, itens e histórico criados em uma única transação.");
  ...
  return <main className="ops-page smart-purchases">
    ...
    <section className="ops-panel"><div className="ops-panel__header"><h2>Pedidos de compra</h2><span>{manager.orders.length} pedido(s)</span></div><div className="ops-table-wrap"><table className="ops-table"><thead><tr><th>Número</th><th>Fornecedor</th><th>Status</th><th>Data</th><th>Previsão</th><th>Itens</th><th>Total</th><th>Comissão</th><th>Ações</th></tr></thead><tbody>{manager.orders.map((row) => <tr key={row.id}><td><strong>{row.numero}</strong></td><td>{row.fornecedor.nome}</td><td><select value={row.status} disabled={!allowedStatusTransitions(row.status).length} onChange={async (event) => { const status = event.target.value; if (!window.confirm(`Confirmar mudança transacional para ${status}?`)) return; try { await manager.status(row, status); setFeedback(`Status alterado para ${status} com histórico.`); } catch (error) { setFeedback(error.message); } }}><option value={row.status}>{row.status}</option>{allowedStatusTransitions(row.status).map((status) => <option key={status}>{status}</option>)}</select></td><td>{row.data}</td><td>{row.previsao || "—"}</td><td>{row.items.length}</td><td>{money(row.valorTotal)}</td><td><strong>{money(getPurchaseOrderCommissionData(row).commission)}</strong></td><td><button onClick={() => setSelected(row)}>Detalhes</button></td></tr>)}</tbody></table></div></section>
    ...
    {selected && <OrderDetails order={selected} manager={manager} suppliers={manager.suppliers} onClose={() => setSelected(null)} onEdit={() => { setOrder({ ...selected, supplier: selected.fornecedor }); setSelected(null); setModal(true); }} onFeedback={setFeedback} />}
...
function OrderModal({ order, setOrder, suppliers, catalog, addProduct, onClose, onSave }) {
  const commission = getPurchaseOrderCommissionData(order);
  return <OperationModal title={order.pcpNeedId ? "Revisar necessidade do PCP" : order.id ? `Editar ${order.numero}` : "Pedido de compra"} editing={Boolean(order.id)} onClose={onClose} onSubmit={onSave} submitLabel={order.id ? "Atualizar pedido" : "Salvar pedido"}><label className="ops-field ops-field--wide"><span>Fornecedor real</span><select value={order.supplier?.id || ""} onChange={(event) => setOrder((current) => ({ ...current, supplier: suppliers.find((supplier) => String(supplier.id) === event.target.value) || null }))}><option value="">Selecione</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.nome}</option>)}</select></label>{[["data", "Data", "date"], ["previsao", "Previsão", "date"], ["condicaoPagamento", "Condição de pagamento"], ["transportadora", "Transportadora"], ["frete", "Frete", "number"], ["desconto", "Desconto", "number"]].map(([key, label, type]) => <label className="ops-field" key={key}><span>{label}</span><input type={type || "text"} value={order[key]} onChange={(event) => setOrder((current) => ({ ...current, [key]: event.target.value }))} /></label>)}<label className="ops-field ops-field--wide"><span>Observações e contexto</span><textarea value={order.observacoes} onChange={(event) => setOrder((current) => ({ ...current, observacoes: event.target.value }))} /></label>{order.pcpNeedId && <div className="ops-preview">Item recebido do PCP. Revise fornecedor, quantidade, preço e condições antes de confirmar.</div>}{order.id && <div className="ops-preview">Cabeçalho, itens e histórico serão sincronizados em uma única transação. Pedidos com recebimento não podem ser reeditados.</div>}<section className="purchase-catalog"><h3>Itens reais do Catálogo Inteligente</h3>{catalog.slice(0, 12).map((product) => <button type="button" key={product.id} onClick={() => addProduct(product)}>{product.supplierCode || product.name}</button>)}</section><section className="purchase-items">{order.items.map((item) => { const itemCommission = getPurchaseItemCommissionData(item); return <article key={item.id}><div><strong>{item.codigo} · {item.descricao}</strong><small>{item.liga} {item.tempera} · {item.estoqueId ? "vinculado ao estoque" : "sem vínculo de estoque"}</small><small>{itemCommission.rate > 0 ? `${commissionRule(itemCommission)} = ${money(itemCommission.commission)}` : "Sem comissão automática"}</small></div><input type="number" min="0.0001" step="any" value={item.quantidade} onChange={(event) => setOrder((current) => ({ ...current, items: current.items.map((candidate) => candidate.id === item.id ? { ...candidate, quantidade: event.target.value } : candidate) }))} /><input type="number" min="0" step="any" value={item.valorUnitario} onChange={(event) => setOrder((current) => ({ ...current, items: current.items.map((candidate) => candidate.id === item.id ? { ...candidate, valorUnitario: event.target.value } : candidate) }))} /></article>; })}</section><div className="ops-preview ops-field--wide"><strong>Comissão total do pedido:</strong> {money(commission.commission)}</div></OperationModal>;
}

function OrderDetails({ order, manager, suppliers, onClose, onEdit, onFeedback }) {
  ...
  const commission = getPurchaseOrderCommissionData(order);
  return <div className="purchase-details"><header><div><small>{order.status}</small><h2>{order.numero}</h2><p>{order.fornecedor.nome} · {money(order.valorTotal)} · comissão {money(commission.commission)}</p></div><div className="ops-actions"><button disabled={order.items.some((item) => item.recebida > 0)} onClick={onEdit}>Editar</button><button onClick={onClose}>×</button></div></header><section><h3>Recebimento e estoque</h3>{order.items.map((item) => { const itemCommission = getPurchaseItemCommissionData(item); return <article key={item.id}><span>{item.codigo} · comprado {item.quantidade} · recebido {item.recebida}<small>{itemCommission.rate > 0 ? `${commissionRule(itemCommission)} = ${money(itemCommission.commission)}` : "Sem comissão automática"}</small></span><button disabled={!item.estoqueId || !["Comprado", "Recebido parcialmente"].includes(order.status) || item.recebida >= item.quantidade} onClick={async () => { const quantity = window.prompt("Quantidade recebida", String(item.quantidade - item.recebida)); if (quantity && window.confirm("Confirmar recebimento e entrada no estoque?")) { try { await manager.receive(item.id, quantity); onFeedback("Recebimento e estoque registrados."); onClose(); } catch (error) { onFeedback(error.message); } } }}>Receber</button></article>; })}</section><section><h3>Cotações comparativas</h3>{order.quotes.map((item) => <p key={item.id}>{item.fornecedor_snapshot.nome} · {money(item.valor_total)} · {item.prazo_dias} dias · {money(item.custo_kg)}/kg</p>)}<select value={quote.supplier?.id || ""} onChange={(event) => setQuote((current) => ({ ...current, supplier: suppliers.find((supplier) => String(supplier.id) === event.target.value) || null }))}><option value="">Fornecedor</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.nome}</option>)}</select>{[["value", "Valor"], ["days", "Prazo"], ["costKg", "Custo/kg"]].map(([key, label]) => <input key={key} type="number" placeholder={label} value={quote[key]} onChange={(event) => setQuote((current) => ({ ...current, [key]: event.target.value }))} />)}<button onClick={async () => { if (!quote.supplier || !window.confirm("Salvar cotação e histórico em uma única transação?")) return; try { await manager.quote(order.id, quote); onFeedback("Cotação e histórico salvos."); onClose(); } catch (error) { onFeedback(error.message); } }}>Adicionar cotação</button></section><section><h3>Financeiro corporativo</h3><input type="number" min="1" value={count} onChange={(event) => setCount(event.target.value)} /><input type="date" value={due} onChange={(event) => setDue(event.target.value)} /><button disabled={!['Aprovado', 'Comprado'].includes(order.status)} onClick={async () => { if (!window.confirm("Sincronizar parcelas pendentes e histórico? Nenhum título financeiro será criado.")) return; try { await manager.installments(order, count, due); onFeedback("Parcelas pendentes e histórico sincronizados."); onClose(); } catch (error) { onFeedback(error.message); } }}>Preparar parcelas</button></section><section><h3>Histórico</h3>{order.history.map((event) => <p key={event.id}><strong>{event.tipo}</strong> · {event.descricao}</p>)}</section></div>;
}
