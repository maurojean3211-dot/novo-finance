...
import { addProductToDraft, deleteImportDraft, detectInternalDuplicates, getImportDraft, getImportSummary, listImportDrafts, saveImportDraft, updateProductInDraft } from "../services/catalogoImportDraft.service";
import { createWzmDemoProducts } from "../services/catalogoDemo.service";
...
export default function useCatalogImports({ companyId, userId }) {
  ...
  const reload = useCallback(() => { if (!companyId || !userId) { setDrafts([]); setLoading(false); return []; } try { const next = listImportDrafts(companyId, userId); setDrafts(next); setError(""); return next; } catch (requestError) { setError(requestError.message || "Erro ao carregar importações."); return []; } finally { setLoading(false); } }, [companyId, userId]);
  useEffect(() => { const timer = window.setTimeout(reload, 0); return () => window.clearTimeout(timer); }, [reload]);
  const selected = useMemo(() => selectedId ? drafts.find((draft) => draft.id === selectedId) || getImportDraft(companyId, userId, selectedId) : null, [companyId, drafts, selectedId, userId]);
  function persist(draft) { const saved = saveImportDraft(draft); reload(); setSelectedId(saved.id); return saved; }
  function remove(id) { const removed = deleteImportDraft(companyId, userId, id); if (selectedId === id) setSelectedId(null); reload(); return removed; }
  function editProduct(draft, productId, changes) { return persist(updateProductInDraft(draft, productId, changes)); }
  function addWzmDemo(draft) { let next = draft; createWzmDemoProducts().forEach((product) => { next = addProductToDraft(next, product); }); return persist(next); }
  ...
  return { drafts, selected, selectedId, setSelectedId, loading, error, reload, persist, remove, editProduct, addWzmDemo, detectDuplicates, markReview, duplicate, metrics, getSummary: getImportSummary };
}
