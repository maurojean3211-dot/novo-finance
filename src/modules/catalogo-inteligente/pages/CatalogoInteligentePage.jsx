...
export default function CatalogoInteligentePage() {
  const { empresaId, userId, ready } = useCompanyScope(); const imports = useCatalogImports({ companyId: empresaId, userId }); const catalog = useCatalogo();
  ...
  function saveMaterial(material) { showDetails(catalog.addMaterial(material)); }
  function deleteDraft(draft) { if (window.confirm(`Excluir a importação “${draft.catalogName || draft.supplierName}”?`)) { imports.remove(draft.id); setFeedback({ type: "success", message: "Importação excluída." }); } }
  function saveDraft(draft) { imports.persist(draft); setImportModal(false); setWorkspace("history"); setFeedback({ type: "success", message: "Rascunho criado e salvo localmente." }); }
  ...
  return <main className="catalog-page">
    ...
    <FeedbackBanner feedback={feedback} onClose={() => setFeedback(null)} />{imports.error && <FeedbackBanner feedback={{ type: "error", message: imports.error }} />}{!ready && <FeedbackBanner feedback={{ type: "error", message: "Sessão incompleta: empresa e usuário precisam estar identificados para acessar rascunhos locais." }} />}
    ...
    <MetricGrid items={[{ label: "Total de importações", value: imports.metrics.total, detail: "rascunhos locais", icon: "▤" }, { label: "Em conferência", value: imports.metrics.review, detail: "status review", icon: "◎", tone: "amber" }, { label: "Produtos extraídos", value: imports.metrics.products, detail: "itens nos rascunhos", icon: "▦" }, { label: "Produtos com alertas", value: imports.metrics.alerts, detail: "exigem conferência", icon: "!", tone: "rose" }, { label: "Duplicidades", value: imports.metrics.duplicates, detail: "possíveis repetições", icon: "◇" }, { label: "Itens aprovados", value: imports.metrics.approved, detail: "selecionados sem alertas", icon: "✓", tone: "green" }]} />
    {imports.selected ? <CatalogImportReview draft={imports.selected} summary={imports.getSummary(imports.selected)} onBack={() => imports.setSelectedId(null)} onEditProduct={(id, changes) => imports.editProduct(imports.selected, id, changes)} onDetectDuplicates={() => { imports.detectDuplicates(imports.selected); setFeedback({ type: "success", message: "Detecção interna de duplicidades concluída." }); }} onLoadDemo={() => { imports.addWzmDemo(imports.selected); setFeedback({ type: "success", message: "Demonstração WZM carregada. Nenhum arquivo foi processado." }); }} onMarkReview={() => imports.markReview(imports.selected)} /> : workspace === "products" ? <><div className="catalog-demo-note"><span /> Produtos demonstrativos existentes · dados mantidos somente em memória</div><MaterialFilters filters={catalog.filters} viewMode={viewMode} onFiltersChange={catalog.updateFilters} onViewModeChange={setViewMode} /><section className="catalog-results"><div className="catalog-results__heading"><div><p className="catalog-eyebrow">Base técnica</p><h2>Materiais cadastrados</h2></div><span>{catalog.filteredMaterials.length} encontrados</span></div><MaterialsView materials={catalog.visibleMaterials} mode={viewMode} onSelect={showDetails} /><CatalogPagination page={catalog.page} pageCount={catalog.pageCount} total={catalog.filteredMaterials.length} onPageChange={catalog.setPage} /></section><FutureIntelligence /></> : <CatalogImportHistory drafts={imports.drafts} loading={imports.loading} onOpen={(id) => { imports.setSelectedId(id); setWorkspace("imports"); }} onDelete={deleteDraft} onDuplicate={(draft) => { imports.duplicate(draft); setFeedback({ type: "success", message: "Rascunho duplicado." }); }} onReview={(draft) => imports.markReview(draft)} onImport={() => setImportModal(true)} />}
