...
export default function BankReconciliationPanel({ empresaId, userId, incomes, expenses, payables = [], onImported }) {
  ...
  return <section className="ops-panel pf-reconciliation"><div className="ops-panel__header"><div><h2>Conciliação bancária por extrato</h2><span>Primeira versão: Nubank PDF · processamento local no navegador</span></div><div className="pf-reconciliation__actions"><input ref={inputRef} hidden type="file" accept="application/pdf,.pdf" onChange={readFile} /><button type="button" onClick={() => inputRef.current?.click()} disabled={busy}>{busy ? "Processando…" : "Selecionar extrato PDF"}</button>{statement && <button type="button" onClick={clearReconciliation} disabled={busy}>Limpar conciliação</button>}<button type="button" className="primary" onClick={() => setConfirmOpen(true)} disabled={busy || !selectedCount}>Importar selecionados ({selectedCount})</button></div></div>
    ...
    {!statement ? <div className="pf-report-empty">Selecione um extrato Nubank para iniciar a conferência.</div> : <>
      ...
      {RECONCILIATION_GROUPS.map((group) => { const records = items.filter((item) => item.situation === group); if (!records.length) return null; return <section key={group} className="pf-reconciliation__group"><header><div><h3>{group}</h3>{group === "Possível duplicidade" && <p>Revise antes de importar. Pode existir um lançamento semelhante no sistema.</p>}</div><span>{records.length} item(ns)</span></header><div className="ops-table-wrap"><table className="ops-table"><thead><tr><th>Importar</th><th>Data</th><th>Descrição do extrato</th><th>Valor</th><th>Tipo sugerido</th><th>Categoria sugerida</th><th>Correspondência</th><th>Situação</th></tr></thead><tbody>{records.map((item) => <tr key={item.id}><td><input type="checkbox" checked={Boolean(item.selected)} disabled={!item.systemType || item.matchedIds.length > 0 || item.payableMatches?.length > 0} onChange={(event) => updateItem(item.id, { selected: event.target.checked })} /></td><td>{item.date.split("-").reverse().join("/")}</td><td>{item.description}</td><td>{money(item.amount)}</td><td><select value={item.suggestedType} onChange={(event) => { const suggestedType = event.target.value; updateItem(item.id, { suggestedType, systemType: suggestedType === "Receita" ? "receita" : ["Despesa", "Investimento"].includes(suggestedType) ? "despesa" : null, selected: false }); }}><option>Receita</option><option>Despesa</option><option>Investimento</option><option>Transferência</option><option>Revisar</option></select></td><td><input value={item.suggestedCategory} onChange={(event) => updateItem(item.id, { suggestedCategory: event.target.value })} /></td><td>{item.payableMatches?.length ? <span className="pf-reconciliation__payable-match">Possível Conta a Pagar: <strong>{item.payableMatches[0].fornecedor || item.payableMatches[0].descricao}</strong>. Confirme a baixa em Contas a Pagar; nenhuma baixa é automática.</span> : "—"}</td><td><select value={item.situation} onChange={(event) => { const situation = event.target.value; updateItem(item.id, { situation, selected: false, matchedIds: ["Faltando lançar", "Investimentos"].includes(situation) ? [] : item.matchedIds }); }}>{RECONCILIATION_GROUPS.map((value) => <option key={value}>{value}</option>)}</select></td></tr>)}</tbody></table></div></section>; })}
    </>}
